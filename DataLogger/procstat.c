...
int getprocstatself(unsigned char printoutput) {
  printf_debug( "getprocstatself: start\n");

  printf_debug( "getprocstatself: tickspersec\n");
  tickspersec = sysconf(_SC_CLK_TCK);

  printf_debug( "getprocstatself: fopen\n");
  ...
  printf_debug( "getprocstatself: start reading\n");
  readone(&pid);
  readstr(tcomm);
  readchar(&state);
  readone(&ppid);
  ...
  readone(&rt_priority);
  readone(&policy);
  printf_debug( "getprocstatself: end reading\n");
...
int getprocmemsize( double *vm_usage, double *resident_set )
{
	printf_debug( "getprocmemsize: start\n");
	if ( getprocstatself(0) == 0)
	{
		printf_debug( "getprocmemsize: start calc\n");
		long page_size_kb = sysconf(_SC_PAGE_SIZE) / 1024;
		*vm_usage = vsize / 1024.0;
		*resident_set = rss * page_size_kb;
		printf_debug( "getprocmemsize: end calc\n");
		return 0;
	}
	else return 1;
}
